...
const freemiumY = fs.readFileSync(path.resolve(__dirname, "../../knowledge/packs/apparel-v1/relevance/freemium.yaml"), "utf8");
const fullY = fs.readFileSync(path.resolve(__dirname, "../../knowledge/packs/apparel-v1/relevance/full_lca.yaml"), "utf8");

const freemiumM = YAML.parse(freemiumY);
const fullM = YAML.parse(fullY);
...
// 3) S kvalifikovaným zdrojem (ecoinvent) musí projít
const sample2 = JSON.parse(JSON.stringify(sample));
sample2.lca.impacts.GWP.meta.sources = [
  { type: "ecoinvent", confidence: 0.72, dqr_total: 1.2, dqr: { ter: 2, ger: 1, tir: 2, perf: 3 } }
];
